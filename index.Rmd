...
date: "*Blog construído com R e R Markdown*"
output: 
  html_document:
    theme: united
    toc: yes
...
```
Há duas opções para ver a saída do programa acima:

1. a cada operação digitada você pode apertar CRTL+ENTER ou simplesmente clicar em RUN como mostra a imagem abaixo para ver a saída do programa;

2. Você também tem a opção de digitar todas essas operações, em seguida selecionar tudo e rodar o programa (apertar CRTL+ENTER ou simplesmente clicar em RUN) para ver a saída.

![](tela.png){width=30cm, height=15cm}
...
### Enunciado 1 - Problema de queda livre 
...
**obs.:** *O símbolo `<-` em R é chamado de operador de atribuição. Ele é usado para atribuir valores a variáveis. Além do operador `<-`, você também pode usar o sinal de igual `=` para atribuição. Ambos fazem a mesma coisa, mas o <- é mais comum na comunidade R.*

2. h <- 1.6: Essa linha define a altura de queda do objeto. A altura é especificada em metros e, no exemplo, é igual a 1,6 metros.

3. a <- 1.6: Nesta linha, estamos atribuindo o valor da aceleração da gravidade na Lua. A aceleração da gravidade na Lua é aproximadamente 1,6 m/s^2^.
...
5. cat("O martelo levaria aproximadamente", round(t, 2), "segundos para alcançar o solo na Lua."):O tempo é arredondado para duas casas decimais e apresentado em segundos. Neste caso, o martelo levaria aproximadamente 1,41s segundos para alcançar o solo na Lua.
...
### Enunciado 2 - Problema de conversão de temperatura
...
Vou analisar o código linha por linha:
...
3. cat("O título do livro é", round(TF, 0), "."): Por fim, exibimos o resultado calculado. A frase “O título do livro é” é seguida pelo valor da temperatura em Fahrenheit, arredondado conforme necessário. Neste caso, o resultado seria algo como “O título do livro é Fahrenheit 451.” (dependendo do arredondamento).

A expressão round(TF, 0) arredonda o valor de TF para o inteiro mais próximo. Isso é útil quando queremos exibir números inteiros em vez de valores com casas decimais.
...
Claro! Vamos explicar o código passo a passo para os alunos do ensino médio:
